...
class VFXMetadataProcessor:
    ...
    def inject_metadata(self, exr_path):
        self.copied_files.append(exr_path)
        ...
        # Preserve existing metadata and add new entries
        for key, value in self.metadata.items():
            if key in existing_metadata:
                print(f"⚠️ Keeping existing metadata for {key}: {existing_metadata[key]}")
            else:
                print(f"✅ Adding new metadata: {key} = {value}")
                existing_metadata[key] = value  # Append new metadata
    ...
    def apply_ocio_conversion(self, input_image, output_image):
        img = oiio.ImageBuf(input_image)
        img = oiio.ImageBufAlgo.colorconvert(img, "ACEScg", "Rec.1886 Rec.709 - Display")
        #img = oiio.ImageBufAlgo.colorconvert(img, "Rec.709", "Output - ACEScg")
        img.write(output_image)
        print(f"Applied OCIO conversion: {input_image} → {output_image}")
    ...
    def convert_sequence_to_video(self, image_sequence, output_video):
        ...
        # Apply ACES LUT if enabled
        # if self.apply_aces:
        #     filter_chain.append("lut3d=aces_to_rec709.cube")
    
        # Apply burn-in overlays if enabled
        if self.burn_in:
            y_position = 100
            for key, value in self.metadata.items():
                filter_chain.append(f"drawtext=text='{key.capitalize()}: {value}': x=100: y={y_position}: fontsize=48: fontcolor=white")
                y_position += 100
    
        # Apply filters (ensures only ONE -vf is used)
        if filter_chain:
    ...
    def process(self):
        ...
        if os.path.isdir(self.path):
            ...
            ocio_exr_sequence = os.path.join(self.path, "ocio_sequence")  # Create a folder for OCIO-converted frames
            os.makedirs(ocio_exr_sequence, exist_ok=True)
            
            for exr_file in exr_files:
                input_exr = os.path.join(self.path, exr_file)
                # output_exr = os.path.join(ocio_exr_sequence, exr_file.replace(".exr", "_ocio.exr"))
                output_exr = os.path.join(ocio_exr_sequence, exr_file)
                self.apply_ocio_conversion(input_exr, output_exr)
                self.copied_files.append(output_exr)
            
            # Use the OCIO-converted sequence in FFmpeg
            exr_sequence = os.path.join(ocio_exr_sequence, self.detect_exr_sequence_pattern())
...
processor.process()
processor.remove_temporary_files()
